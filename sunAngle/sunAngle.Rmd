---
title: "Create sun location world map"
author: "Neil Kaye"
date: "9/29/2020"
...
## Introduction
This is an example of some code to create a map of the angle of the sun on a specific day of the year
...
```
We will load some world country data
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=5.5}
data(wrld_simpl)
```
As ggplot needs a data frame in order to create a map here is a simple function to convert 
a raster into a data frame
```{r, message=FALSE, warning=FALSE}
...
```
I use this backSet object in order to remove all axes legends and panels to create a completely
blank background to plot one
```{r, message=FALSE, warning=FALSE}
...
```
Here we create a raster of the globe and populate it with angle of the sun values
for each raster cell for a specific time
...
Show only a subset area of the data
```{r  , message=FALSE, warning=FALSE, fig.width=10, fig.height=5.5}
...
Show map as a different projection
```{r , message=FALSE, warning=FALSE, fig.width=10, fig.height=5.5}
